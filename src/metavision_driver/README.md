...
- ``send_queue_size``: outgoing ROS message send queue size (defaults
  to 1000 messages).
- ``input_queue_size``: maximum number of raw camera callback blocks held by
  the multithreaded processing queue (defaults to 4). When full, new blocks
  are dropped and reported in the periodic statistics log.
