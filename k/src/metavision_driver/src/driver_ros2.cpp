    ...
    rclcpp::NodeOptions(options).automatically_declare_parameters_from_overrides(true))
{
  ...
  const bool is_evt21 = encoding_ == "evt21" || encoding_.rfind("evt21;", 0) == 0;
  if (!is_evt3 && !is_evt21) {
...
void DriverROS2::start()
{
  ...
  int64_t inputQueueSize;
  this->get_parameter_or("input_queue_size", inputQueueSize, int64_t(4));
  if (inputQueueSize < 1) {
    throw std::runtime_error("input_queue_size must be at least 1");
  }
  wrapper_->setInputQueueSize(static_cast<size_t>(inputQueueSize));
